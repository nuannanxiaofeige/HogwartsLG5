...
class  App(BasePage):
    def start(self,appium_port=4723,udid="127.0.0.1:62001"):
        ...
        if self.driver == None:
            ...
            desired_caps['appActivity'] = '.view.WelcomeActivityAlias'  # 从当前页面开始点击
            # 不清空本地缓存，启动app
            desired_caps["noReset"] = "true"
            desired_caps['udid'] =  udid
            # desired_caps["dontStopAppOnReset"]='true',  # 不退出应用
            desired_caps["skipDeviceInitialization"] = 'true'  # 跳过一些安装、权限的步骤，提升运行速度
            self.driver = webdriver.Remote(f"http://localhost:{appium_port}/wd/hub", desired_caps)
